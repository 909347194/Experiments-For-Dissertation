# -*- coding: utf-8 -*-
"""plot_llm_decisions.py — LLM 决策时间线可视化。

生成图表：
1. LLM 决策时间线：每代的策略选择和 CR 值变化
2. LLM 调用耗时分布
3. 策略选择频率饼图
"""
...
from ._common import COLORS, _sanitize_filename, setup_chinese_font
...
class LLMDecisionPlotter:
    ...
    def plot_decision_timeline(
        self,
        llm_decisions: dict[int, list[dict]],
        scenario_name: str = "",
    ) -> Path:
        """绘制 LLM 决策时间线。

        X 轴：代数，Y 轴左：策略选择，Y 轴右：CR 值。
        每个 run 一条线，展示 LLM 在不同代的决策变化。

        Args:
            llm_decisions: {run_idx: [decision_dict, ...]}
            scenario_name: 场景名（用于标题和文件名）。
        """
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=self.figsize, sharex=True)
        fig.suptitle(f"LLM Decision Timeline — {scenario_name}", fontsize=14)

        strategy_map = {"rand/1": 0, "best/2": 1, "default": 0.5}
        colors = plt.cm.tab10(np.linspace(0, 1, min(10, len(llm_decisions))))
