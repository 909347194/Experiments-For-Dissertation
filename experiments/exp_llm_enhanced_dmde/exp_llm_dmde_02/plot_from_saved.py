# -*- coding: utf-8 -*-
"""plot_from_saved.py — 读取已保存的 exp_llm_dmde_02 数据重新绘图

背景：
    ``run.py`` 每次运行需完成 LLM 增强 DMDE 求解，并把绘图所需的全部数据
    （含 LLM 决策日志）保存为 ``results/exp_llm_dmde_02_data.json``。
    本脚本只负责：加载该 JSON → 重建可视化对象 → 调用 visualizer 出图，
    因此调整绘图样式时无需重跑实验。

用法：
    # 默认：加载 results/exp_llm_dmde_02_data.json
...
DEFAULT_DATA_FILE = RESULTS_DIR / "exp_llm_dmde_02_data.json"
